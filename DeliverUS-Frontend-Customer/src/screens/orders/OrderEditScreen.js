/* EditOrderScreen de PAULA (el de Rosa va mejor)
...
export default function OrderEditScreen ({ navigation, route }) {
  ...
  const [productosdeseados, setproductosdeseados] = useState(new Map()) // Lo inicializamos como un Map
  ...
  useEffect(() => {
    const fetchProductsDeseados = () => {
      for (const product of route.params.order.products) { // por cada producto del order
        productosdeseados.set(product.id, product.OrderProducts.quantity) //metemos una nueva pareja clave-valor al Map siendo la clave el id del producto y el valor la cantidad de este mismo
      }
    }
  ...
  const updateOrder = async (values) => {
    setBackendErrors([])
    const productos = []
    for (const product of productosdeseados.keys()) { // recorremos las claves del Map
      productos.push({ // vamos llenando el array que hemos inicializado vacío con los nuevos datos
        productId: product,
...
// OrderEditScreen de ROSA (va mejor que el de Paula)
